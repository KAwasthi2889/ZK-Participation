...
Parameter format:
All Bytes<32> values are passed as Uint8Array(32).

CLI currently uses toBytes32() helper.
...
Patient:
  Holds credentialData + secret

Current limitation:
Verifier currently receives credentialData + secret.

This is MVP behavior.
Future versions may replace this with a true ZK proof sharing flow.
...
2. `npm run compile` — compiles `contracts/credential.compact` to `contracts/managed/credential/`.
...
cipher/
├── contracts/
│   └── credential.compact     # Main Compact source
│   └── hello-world.compact     # Example Compact source
├── scripts/
