/**
 * CLI for interacting with cipher contract
 */
...
async function main() {
  ...
  try {
    ...
    while (running) {
      ...
      console.log("  4. Revoke Credential");
      ...
      switch (choice.trim()) {
        case "1": {
          const issuerSecret = await rl.question("  Issuer Secret: ");

          console.log("\n  Registering issuer...");

          try {
            const tx = await deployed.callTx.registerIssuer(
              toBytes32(issuerSecret),
            );
        ...
        case "2": {
          const credentialData = await rl.question("  Credential Data: ");

          const secret = await rl.question("  Credential Secret: ");
          ...
          try {
            ...
            if (message.includes("Not registered issuer")) {
        ...
        case "3": {
          const credentialData = await rl.question("  Credential Data : ");
          const secret = await rl.question("  Secret : ");

          console.log("\n  Verifying credential...");

          try {
            const tx = await deployed.callTx.verifyProof(
              toBytes32(credentialData),
              toBytes32(secret),
            );
            ...
            } else if (message.includes("Credential not issued")) {
        ...
        case "4": {
          ...
          try {
            const tx = await deployed.callTx.revokeCredential(
              toBytes32(credentialData),
              toBytes32(secret),
              toBytes32(issuerSecret),
            );
            ...
            if (message.includes("Not registered issuer")) {
              console.log("\n  ❌ Not registered issuer\n");
            } else if (message.includes("Credential not found")) {
              console.log("\n  ❌ Credential not found\n");
            } else {
              console.error("\n  ❌ Verification failed:", message);
            }
